...
**2026-09-21追記**: 実装(`app/trading/application/trading_halt.py`)が発動元として
接続したのは10原因中2つ(データ遅延、日次/週次損失・DD上限)のみ。スコープの経緯は
`decisions/0004-trading-halt-mvp-scope.md`を参照。
...
**2026-09-21追記**: 以下の`release_pending`経由の是非という未決定事項は、
`decisions/0004-trading-halt-mvp-scope.md`で「MVPでは`active`/`released`の2値のみ
使用し`release_pending`は経由しない」と決定された。以下の表自体は経緯の記録として
残す(実装がこの表の一部のみを採用したことは同ADRを参照)。
...
~~`stopping` / `error`~~ `stopping` / `failed`

**2026-09-20訂正**: `actual_state`の終端失敗値は、Bot lifecycle実装
(`app/trading/application/bot_lifecycle.py`)時に実DBの`trading_bot.actual_state`
CHECK制約を確認した結果、上記`error`ではなく**`failed`**であることが判明した
(`database/postgresql_schema_v0.1.sql`)。実DBを正とする既存方針により、
実装は`failed`を用いる。
...
**2026-09-20補足**: 上表は`start`を`stopped`/`paused`両方から許可しているが、
`bot_lifecycle.py`の実装では`start`は`stopped`からのみ、`resume`は`paused`
からのみを許可する、より狭い区分を採用した(`paused`のBotへの`start`は
`409 Conflict`ではなく`invalid_state`エラーとして拒否し、`resume`を使うよう促す)。
2つの独立したコマンドとして扱う方が、それぞれの責務(新規`bot_run`作成 vs
既存`bot_run`継続)を明確に保てるための実装判断であり、詳細は同モジュールの
docstring参照。
...
**2026-09-20追記(「損失回避優先ロジックの実装上の具体化」への回答)**: Bot lifecycle
実装時、この要件は実際にはstop-loss/take-profitの**監視・執行エンジンそのもの**の
継続稼働を指しており、`app/trading/application/dummy_pipeline.py`のダミーシグナル
(方向性シグナル)とは別物であることが判明した。本システムには現時点でそのような
監視・執行エンジン自体が存在しない(`trade_order.stop_price`/`take_profit_price`は
列として保持されるが、Bot状態に関わらずどこからも参照・執行されない、
`order_flow.py`のdocstringに既に記録済みの既存の欠落)。そのため`bot_lifecycle.py`
自身がpause中に継続すべき対象は無い。一方、依頼されたタスクの記述は「新規建て玉は
行わないが、既存ポジションの決済シグナルは引き続き評価する」という、より限定された
最小要件を明示していたため、`dummy_pipeline.py`側でその区別(pause中も既存
ポジションに逆行するシグナルへの決済のみは許可し、新規/同方向の建玉は抑止する)を
実装した。本表冒頭の「pause実行時の新規発注: 新しいシグナル評価...を停止する」を
文字通り解釈すると決済シグナルの評価も止まるように読めるが、上記の理由により
「新規建玉につながるシグナル評価」に限定して読むのが正しい、というのが今回の
実装判断である。
